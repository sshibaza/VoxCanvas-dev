...
export function createSetupRouter(scrt2Client) {
  ...
  router.post('/setup/complete', (req, res) => {
    try {
      const { consumerKey, username, loginUrl, scrtBaseUrl, orgId, callCenterApiName, callCenterPhone } = req.body;
      if (!consumerKey || !username || !scrtBaseUrl || !orgId || !callCenterApiName) {
        return res.status(400).json({
          error: true,
          code: 'MISSING_FIELDS',
          message: 'consumerKey, username, scrtBaseUrl, orgId, and callCenterApiName are required.',
        });
      }

      const safe = {
        consumerKey: sanitizeEnvValue(consumerKey, 'consumerKey'),
        username: sanitizeEnvValue(username, 'username'),
        loginUrl: sanitizeEnvValue(loginUrl || 'https://login.salesforce.com', 'loginUrl'),
...
SF_CONSUMER_KEY=${safe.consumerKey}
SF_USERNAME=${safe.username}
SF_LOGIN_URL=${safe.loginUrl}
...
`;
      ...
      // Apply to the running client so the user doesn't have to restart
      // just to exercise /api/health or /api/voice-call.
